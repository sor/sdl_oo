...
namespace SDL
{
		...
		: public C::SDL_Color
	{
		...
		__alwaysinline
		explicit
		Color( Uint8 rgb ) noexcept
		{
			this->r =
			this->g =
			this->b = rgb;	// all set to rgb (grey)
			this->a = 255;
		}

		__alwaysinline
		Color( Uint8 r, Uint8 g, Uint8 b ) noexcept
		{
			this->r = r;
			this->g = g;
			this->b = b;
		...
		// unportable, VS2010 does not support, because of that its untested right now
		__alwaysinline
		explicit Color( std::initializer_list<Uint8> list ) noexcept
		{
			auto it = list.begin();
			if( list.size() == 1 )
			{
